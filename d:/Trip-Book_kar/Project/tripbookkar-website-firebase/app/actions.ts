...
export async function saveInterestAction(
  ...
  try {
    ...
    await interestRef.set(processedData, { merge: true });
...
export async function savePackageAction(
  ...
  try {
    const isNew = !existingPackageId;
    const packageId = existingPackageId || slugify(data.name);

    if (!packageId) {
      throw new Error("Package name is required to create an ID.");
    }
    ...
    await packageRef.set(packageData, { merge: true });

    revalidatePath('/admin/packages');
    revalidatePath(`/packages/${packageId}`);
    revalidatePath('/');

    return { success: true, message: "Package saved successfully.", isNew };
  } catch (error: any) {
    console.error("Error saving package:", error);
    return { success: false, message: error.message, isNew: !existingPackageId };
  }
}
