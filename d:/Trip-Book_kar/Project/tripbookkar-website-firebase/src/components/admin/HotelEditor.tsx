...
export default function HotelEditor({ hotel }: HotelEditorProps) {
  ...
  const handleGenerateDetails = async (sections: ('about' | 'dining' | 'experiences' | 'weddings' | 'tags' | 'rooms')[]) => {
    ...
    if (result.details) {
      ...
      if(sections.includes('experiences')) setValue('experiencesAndActivities', formatForFieldArray(result.details.experiencesAndActivities || []));
      if(sections.includes('weddings')) setValue('weddingVenues', formatForFieldArray(result.details.weddingVenues || []));
      if(sections.includes('tags')) setValue('tags', formatForFieldArray(result.details.tags || []));
             ...
             <section>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button type="button" variant="outline" onClick={() => appendExperience({ value: '' })}>
                           Add Activity
                        </Button>
                    </div>
                </div>
                    ...
                    {experienceFields.map((field, index) => (
                         <div key={field.id} className="grid grid-cols-[1fr,auto] gap-4 items-end p-4 border rounded-lg bg-secondary/30">
                            <div className="space-y-2">
                                ...
                                <Input {...register(`experiencesAndActivities.${index}.value`)} placeholder="e.g., Rooftop Pool" />
                            </div>
             ...
             <section>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button type="button" variant="outline" onClick={() => appendWedding({ value: '' })}>
                            Add Venue
                        </Button>
                    </div>
                </div>
                <div className="space-y-4">
                    {weddingFields.map((field, index) => (
                        <div key={field.id} className="grid grid-cols-[1fr,auto] gap-4 items-end p-4 border rounded-lg bg-secondary/30">
                            <div className="space-y-2">
                                <Label>Venue</Label>
                                <Input {...register(`weddingVenues.${index}.value`)} placeholder="e.g., Grand Ballroom" />
                            </div>
                ...
                <div className="space-y-4">
                    {tagFields.map((field, index) => (
                         <div key={field.id} className="grid grid-cols-[1fr,auto] gap-4 items-end">
                            <Input {...register(`tags.${index}.value`)} placeholder="e.g., luxury" />
